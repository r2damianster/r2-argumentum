...
- Si una pestaña de host se pierde/recarga y no puedes recuperar la sesión, lo más simple es abrir una sesión **nueva** (nuevo código de sala) y hacer que los participantes reingresen con `?sala={codigoNuevo}`, en vez de pelear por recuperar la vieja.
...
- **"Iniciar sesión"** (botón del host): sortea co-moderadores (`ceil(n×0.10)`, mínimo 1, pero nunca más de `n-2` para garantizar al menos 2 argumentadores) y asigna posturas (solo las elegidas en el selector) al resto, arranca la fase **`Apertura simultánea`** (no directo a la ruleta de turnos).
- **Autoselección de postura**: si el Programa tiene `asignacionPostura: "libre"` (el de 12 posturas filosóficas la usa), el participante ve una pantalla bloqueante "Elige la postura que vas a defender" en vez de que se la asignen al azar — no puede hacer nada más hasta elegir. Los co-moderadores nunca la ven.
...
- **Feed de actividad narrado** (host y player): banner "🗣️ {nombre} está hablando ahora" mientras alguien tiene el turno en curso, "⏳ Se le ofreció el turno a {nombre}…" mientras espera aceptación, y una lista de "Actividad reciente" ("💬 {nombre} agregó un contraargumento", "🔗 {nombre} conectó su argumento (refuerzo)"). Instrucciones explícitas en el turno ofrecido ("Acepta para escribir un argumento nuevo…") y en el formulario de argumento (qué tipos puedes usar). Aviso de anonimidad en el panel de co-moderador al votar bids y validar argumentos.
...
Estos 13 ya se arreglaron en sesiones de prueba anteriores. Si alguno reaparece, es una regresión real y sí va en la tabla de fallos:
...
13. **Con pocos participantes, nadie podía escribir nada** — el sorteo de co-moderadores (`ceil(n×0.10)` mínimo 1) podía convertir a TODOS los presentes en co-moderadores (con 1 participante, esa única persona quedaba de co-moderadora, cero argumentadores). Confirmado y arreglado: ahora nunca se sortean más de `participantesElegibles.length - 2` co-moderadores. **Probar con 2 participantes exactos**: ninguno debe quedar como co-moderador (0 co-moderadores con n=2), ambos deben poder escribir su argumento de apertura.
...
- **Autoselección de postura.** Usar el Programa "¿Qué hace único al ser humano?" (ahora es `asignacionPostura: "libre"`). Tras "Iniciar sesión", cada participante NO co-moderador debe ver una pantalla bloqueante "Elige la postura que vas a defender" con las posturas elegidas por el host como botones — nada de formulario de argumento ni feed hasta elegir. Confirmar que el co-moderador nunca ve esta pantalla.
...
- Si alguno de los 13 bugs de la sección 3 reaparece, marcalo como "REGRESIÓN" y ponelo primero en la tabla, severidad alta.
